...
1. LSTM microservice will accept the GitHub data from Flask microservice and will forecast the data for next 1 year based on past 2 months(60 days)
...
def statmisc():
    ...
    if df.empty:
        return jsonify({"error": "No valid closed_at dates"}), 400
...
def pulls():
    ...
    '''
    Look back decides how many days of data the model looks at for prediction
    Here LSTM looks at approximately two month data
    '''
    look_back = 30
    X_train, Y_train = create_dataset(train, look_back)
    X_test, Y_test = create_dataset(test, look_back)
